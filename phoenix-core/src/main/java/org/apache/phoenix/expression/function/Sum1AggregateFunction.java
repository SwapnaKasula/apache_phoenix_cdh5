...
import org.apache.phoenix.parse.SumAggregateParseNode1;
 ...
 * @since 0.1
 */
@BuiltInFunction(name= Sum1AggregateFunction.NAME, nodeClass=SumAggregateParseNode1.class, args= {@Argument(allowedTypes={PDecimal.class})} )
public class Sum1AggregateFunction extends DelegateConstantToCountAggregateFunction {
    public static final String NAME = "SUM1";

    public Sum1AggregateFunction() {
    }

    // TODO: remove when not required at built-in func register time
    public Sum1AggregateFunction(List<Expression> childExpressions){
        super(childExpressions, null);
    }

    public Sum1AggregateFunction(List<Expression> childExpressions, CountAggregateFunction delegate){
        super(childExpressions, delegate);
    }
    
    private Aggregator newAggregator(final PDataType type, SortOrder sortOrder, ImmutableBytesWritable ptr) {
        //System.out.println("*******************   inside my newAggregator ****************");
        if (type == PDecimal.INSTANCE) {
            //System.out.println("*******************   inside my newAggregator DecimalSumAggregator ****************");
...
//        }
          ...
          //System.out.println("*******************   inside my newAggregator NumberSumAggregator ****************");
